...
assessmentButton.onclick=()=>{//function()と同義→アロー関数
  const userName=userNameInput.value;
  if(userName.length===0){
    //名前が空のときは処理を終了する
    return; //関数の処理の中で、 return; は、戻り値なしにそこで処理を終了するという意味です。
  }
...
const anchor=document.createElement('a');//←顔みたいで可愛い
const hrefValue=
...
const answers=[//const→再代入出来ない変数＝定数
